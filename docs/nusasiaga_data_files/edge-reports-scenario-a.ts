 ...
 * responder's phone. This is the input data the command-center Gemma 4 31B
 * model synthesizes into a CommandCenterSynthesis.
 *
 * Derived from `data/synthesis_scenarios/scenario_a_jakarta_flood.json` in
 * the gemma-disaster-grid repo. Re-export here as a typed TS module so
 * NusaSiaga can `import { reports } from "@/lib/reports"` without any
 * runtime JSON parsing.
 *
 * COPY THIS FILE TO: nusasiaga/src/lib/reports.ts
 */

import type { EdgeTriageReport } from "./types";

export const reports: EdgeTriageReport[] = [
  {
