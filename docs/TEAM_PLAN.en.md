...
- Model downloaded on the test phone. **First multimodal inference confirmed working** on Samsung Galaxy A71 (a 2020 mid-range phone, Snapdragon 730, no NPU). Inference is CPU-bound and takes ~20-60 sec per response on this hardware — slow by flagship standards, deliberate by Global Resilience standards. **That is the demo device.**
...
- [x] Run the unmodified gallery on phone; confirm Gemma 4 E2B loads and runs — verified on Samsung Galaxy A71 (2020 Snapdragon 730, no NPU): multimodal image-to-text inference works, ~20-60 sec per response on this hardware class. The slowness is intentional demo material for the Global Resilience pitch.
...
- [x] **Stretch goal hit same day — voice input.** Added voice as a first-class input alongside photo so the responder can triage via a photo, a voice note, or both. Required: rewriting the system prompt to handle all three modality combinations without breaking the validated JSON contract (rules for "counting people" branch by modality; "trust the image" is now scoped to the both-present case); flipping `supportAudio = true` on init so LiteRT-LM loads the audio backend; adding inline `AudioRecord` capture at 16 kHz mono PCM 16-bit; wrapping the captured PCM in a 44-byte RIFF/WAVE header before sending to the model (LiteRT-LM's `miniaudio` decoder rejects raw PCM with error -10 — discovered on first run, header layout mirrored from the gallery's own `ChatMessageAudioClip.genByteArrayForWav()`); `RECORD_AUDIO` permission flow; 30-second hard cap; recording UI with elapsed-time indicator; `DisposableEffect` to release the `AudioRecord` on screen exit; result label adapts to inputs. **Verified on Galaxy A71 in all three modes (photo-only / voice-only / both)**, all producing valid `EdgeTriageReport` JSON.
- [x] (Newbie was slow; user forked the dashboard to `listyantidewi1/nusasiaga` and deployed it) Vercel live at <https://nusasiaga.vercel.app> with `NASA_FIRMS_MAP_KEY` configured.

### Day 3 overshoot — 15 May (ahead of schedule, all extra)

*Day 3 closed early so we kept shipping. Everything below was outside the original 6-day plan but is now live and verified. The hackathon ship state at end of day is: a fully working phone → dashboard pipeline with offline queue, real environmental data on the dashboard, and a properly rebranded Android app.*

- [x] **Phone → dashboard live sync.** New `/api/reports` route on the dashboard (Next.js App Router, POST + GET handlers, shared-secret auth via `X-Triage-Token` header, schema validation). Storage backed by **Upstash Redis** provisioned via the Vercel Marketplace (note: Vercel KV is deprecated as of 2026-05; Upstash is the forward path, env vars `KV_REST_API_URL` / `KV_REST_API_TOKEN` are auto-injected). Android `TriageUploader` POSTs the report immediately after triage finalises. Dashboard polls `/api/reports` every 10 s through a `useLiveReports` hook and renders new reports in an `IncomingReportsPanel` plus pulsing pins on the operational map. End-to-end verified across three continents (Jakarta, Karachi, Porto Alegre) via curl smoke tests.
- [x] **Offline-first sync stack.** SharedPreferences-backed `TriageQueue` (each pending report keyed by `report_id`; survives app kill / device reboot). `TriageSyncManager` with three retry triggers: foreground (after each triage), `ConnectivityManager.NetworkCallback` (drains the moment validated internet appears), and `WorkManager` (`SyncWorker` as plain `CoroutineWorker`; expedited one-time on triage + 30-min periodic catch-up, both gated by `NetworkType.CONNECTED`). Hard errors (4xx) drop from queue so a poison record doesn't loop forever; transient errors (5xx / network) leave it queued. New `SyncState.Queued(reason)` UI state with amber chip + retry button. Pending-count pill in the screen header.
- [x] **GPS + reverse geocoding** on every uploaded report. `FusedLocationProviderClient` with `CurrentLocationRequest` (4 s timeout, balanced-power-accuracy), `Geocoder` to produce a `subLocality, adminArea, country` label. The dashboard map drops a pin at the responder's actual location instead of showing raw coordinates.
- [x] **App rebrand.** Launcher name "Gemma Rescue Grid"; red diagonal-gradient adaptive icon with a white warning triangle (custom vector drawable). Single-tile home: filtered the gallery's Hilt `Set<CustomTask>` down to `disaster_triage` only. Required defensive patches in `HomeScreen.kt` (`gm4=false` + `listOfNotNull` for the highlighted-tiles section that previously hard-required Chat/AgentChat) and the manifest (`android:label` was a hardcoded literal, switched to `@string/app_name`).
- [x] **Dashboard restructure.** Dropped the four-way scenario picker. New two-mode toggle: **Triage Operations** (Scenario A synthesis + unified operational map) / **Wildfire Monitoring** (NASA FIRMS view). The operational map now merges reports from all three pre-baked scenarios with live phone uploads, filterable via a 10-chip disaster-type bar. Auto-fit-to-all toggle on the map zooms to the bounds of every visible pin (worldwide if reports come from multiple continents). Map pins for live reports are pulsing rings; scenario reports are solid circles — visually distinct.
- [x] **Global FIRMS.** Bbox widened from Indonesia-only (`95,-11,141,6`) to worldwide (`-180,-90,180,90`). Added coarse continental fallback regions so hotspots outside Indonesia get a sensible `region / subregion` label. Cap at top-500-FRP so the browser doesn't try to mount 50 k+ markers. Fixed the SSR loopback `fetch('http://localhost:3000/api/firms')` build-time error by short-circuiting on `NEXT_PHASE === 'phase-production-build'` and using `VERCEL_URL` for the runtime base.
- [x] **EnvironmentStats wired to real data.** New `/api/environment` route proxies the **Open-Meteo** free air-quality + weather endpoints (no API key). AQI + wind + smoke-spread risk anchored at the **viewer's geolocation** (browser permission → IP fallback via ipapi.co → Jakarta default). Carbon release computed from FIRMS FRP totals with a peatland weighting. Emergency priority bucket derived from severity counts. Active-hotspots card replaces the bogus "Response Readiness" demo metric.
- [x] **IncidentFeed pivoted to real.** Was five hardcoded Indonesian strings; now scores every FIRMS hotspot by `severity_score + risk_score - distance/100` from the viewer's location and surfaces the top 5. Titles, regions, and response-status text are derived from FIRMS fields. Empty state handled.
- [x] **UX cleanup.** Removed `DemoReadinessPanel` (internal hackathon checklist that shouldn't be in user UI), `HazardAnalysisPanel` (Ollama-based local AI analyzer — redundant now that the phone is the real local AI), and `LocalAiMode` banner from the wildfire view. Locality references ("Jakarta", "Cianjur") stripped from scenario labels and meta descriptions so the platform reads as global.
