...
- [x] **Stretch goal yang berhasil di-deliver pada hari yang sama — voice input.** Voice ditambahkan sebagai input first-class berdampingan dengan photo sehingga responder dapat melakukan triage menggunakan photo, voice note, atau keduanya. Pekerjaan yang dilakukan: menulis ulang system prompt agar dapat menangani ketiga kombinasi modalitas tanpa merusak kontrak JSON yang telah divalidasi (rules "counting people" sekarang bercabang berdasarkan modalitas; rule "trust the image" kini hanya berlaku pada kasus keduanya hadir); mengaktifkan `supportAudio = true` pada init agar LiteRT-LM melakukan load audio backend; menambahkan inline `AudioRecord` capture pada 16 kHz mono PCM 16-bit; melakukan wrap pada PCM yang ter-capture dengan header RIFF/WAVE 44-byte sebelum dikirim ke model (decoder `miniaudio` pada LiteRT-LM menolak raw PCM dengan error -10 — ditemukan pada percobaan pertama, layout header diadaptasi dari `ChatMessageAudioClip.genByteArrayForWav()` milik gallery sendiri); flow permission `RECORD_AUDIO`; batas keras 30 detik; UI perekaman dengan indikator waktu berjalan; `DisposableEffect` untuk melepas `AudioRecord` saat keluar screen; label result yang adaptif berdasarkan input. **Telah diverifikasi pada Galaxy A71 untuk seluruh ketiga mode (photo-only / voice-only / both)**, seluruhnya menghasilkan JSON `EdgeTriageReport` yang valid.
- [x] (Newbie terlambat; user melakukan fork dashboard ke `listyantidewi1/nusasiaga` dan melakukan deploy sendiri.) Vercel telah live di <https://nusasiaga.vercel.app> dengan `NASA_FIRMS_MAP_KEY` ter-konfigurasi.

### Hari 3 overshoot — 15 Mei (di luar rencana awal, seluruhnya bonus)

*Hari 3 selesai lebih awal sehingga pengerjaan dilanjutkan. Seluruh item di bawah ini berada di luar rencana 6 hari semula namun sekarang telah live dan terverifikasi. State akhir hari: pipeline phone → dashboard yang berfungsi penuh dengan offline queue, data lingkungan riil pada dashboard, serta aplikasi Android yang telah di-rebrand secara menyeluruh.*

- [x] **Sinkronisasi live phone → dashboard.** Route `/api/reports` baru pada dashboard (Next.js App Router, handler POST + GET, autentikasi shared-secret melalui header `X-Triage-Token`, validasi schema). Penyimpanan menggunakan **Upstash Redis** yang disediakan melalui Vercel Marketplace (catatan: Vercel KV telah deprecated per 2026-05; Upstash adalah jalur ke depan, env vars `KV_REST_API_URL` / `KV_REST_API_TOKEN` ter-inject secara otomatis). `TriageUploader` pada Android melakukan POST report segera setelah triage finalisasi. Dashboard melakukan polling pada `/api/reports` setiap 10 detik melalui hook `useLiveReports` dan melakukan render report baru pada `IncomingReportsPanel` serta pin berdenyut pada operational map. End-to-end telah diverifikasi melintasi tiga benua (Jakarta, Karachi, Porto Alegre) melalui curl smoke test.
- [x] **Stack sinkronisasi offline-first.** `TriageQueue` berbasis SharedPreferences (setiap report pending di-key oleh `report_id`; bertahan melalui app kill / reboot perangkat). `TriageSyncManager` dengan tiga trigger retry: foreground (setelah setiap triage), `ConnectivityManager.NetworkCallback` (melakukan drain pada saat internet ter-validasi muncul), dan `WorkManager` (`SyncWorker` sebagai plain `CoroutineWorker`; expedited one-time pada triage + periodic 30 menit untuk catch-up, keduanya dengan gating `NetworkType.CONNECTED`). Hard error (4xx) dihapus dari queue agar poison record tidak ter-loop selamanya; transient error (5xx / network) tetap berada dalam queue. State UI baru `SyncState.Queued(reason)` dengan chip amber + tombol retry. Pill jumlah pending pada header screen.
- [x] **GPS + reverse geocoding** pada setiap report yang ter-upload. `FusedLocationProviderClient` dengan `CurrentLocationRequest` (timeout 4 detik, balanced-power-accuracy), `Geocoder` untuk menghasilkan label `subLocality, adminArea, country`. Map dashboard meletakkan pin pada lokasi aktual responder alih-alih menampilkan koordinat mentah.
- [x] **Rebrand aplikasi.** Nama launcher "Gemma Rescue Grid"; adaptive icon dengan red diagonal-gradient dan warning triangle putih (vector drawable kustom). Tampilan home dengan satu tile saja: melakukan filter pada Hilt `Set<CustomTask>` milik gallery hingga hanya menyisakan `disaster_triage`. Diperlukan patch defensif pada `HomeScreen.kt` (`gm4=false` + `listOfNotNull` untuk section highlighted-tiles yang sebelumnya hard-require Chat/AgentChat) dan manifest (`android:label` yang sebelumnya literal hardcoded, di-switch ke `@string/app_name`).
- [x] **Restrukturisasi dashboard.** Picker scenario empat-arah dihapus. Toggle dua-mode baru: **Triage Operations** (sintesis Scenario A + operational map terunifikasi) / **Wildfire Monitoring** (view NASA FIRMS). Operational map sekarang melakukan merge report dari ketiga pre-baked scenario dengan upload live phone, dapat difilter melalui chip bar disaster-type berisi 10 chip. Toggle auto-fit-to-all pada map melakukan zoom ke bounds setiap pin yang terlihat (worldwide jika report datang dari beberapa benua). Pin map untuk live report berbentuk ring berdenyut; report scenario berupa lingkaran solid — secara visual berbeda.
- [x] **FIRMS global.** Bbox diperluas dari hanya-Indonesia (`95,-11,141,6`) menjadi worldwide (`-180,-90,180,90`). Penambahan fallback region kontinental kasar agar hotspot di luar Indonesia memperoleh label `region / subregion` yang sensible. Cap pada top-500-FRP agar browser tidak mencoba melakukan mount 50 ribu+ marker. Perbaikan error build SSR loopback `fetch('http://localhost:3000/api/firms')` melalui short-circuit pada `NEXT_PHASE === 'phase-production-build'` serta penggunaan `VERCEL_URL` untuk base runtime.
- [x] **EnvironmentStats dihubungkan ke data riil.** Route `/api/environment` baru sebagai proxy untuk endpoint **Open-Meteo** air-quality + weather gratisan (tanpa API key). AQI + wind + smoke-spread risk di-anchor pada **geolocation viewer** (permission browser → IP fallback via ipapi.co → default Jakarta). Carbon release dihitung dari total FRP FIRMS dengan pembobotan peatland. Bucket emergency priority diturunkan dari severity count. Card active-hotspots menggantikan metric demo "Response Readiness" yang fake.
- [x] **IncidentFeed dialihkan ke data riil.** Sebelumnya lima string hardcoded Indonesia; sekarang setiap hotspot FIRMS di-score dengan `severity_score + risk_score - distance/100` dari lokasi viewer dan top 5 yang muncul. Title, region, dan teks response-status diturunkan dari field FIRMS. Empty state telah ditangani.
- [x] **UX cleanup.** Penghapusan `DemoReadinessPanel` (checklist internal hackathon yang seharusnya tidak berada pada UI user), `HazardAnalysisPanel` (analyzer AI lokal berbasis Ollama — redundan karena phone sekarang adalah AI lokal yang sesungguhnya), dan banner `LocalAiMode` dari view wildfire. Referensi lokalitas ("Jakarta", "Cianjur") dihapus dari label scenario dan meta description sehingga platform terbaca sebagai bersifat global.
