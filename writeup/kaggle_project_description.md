...
**Synthesis tier.** A Kaggle notebook (`notebook/gemma_rescue_grid_kaggle.ipynb` in the repo) loads Gemma 4 31B via Unsloth `FastModel` with `device_map="balanced"`, processes all three pre-curated scenarios (12, 15, 8 reports respectively), and emits drop-in TypeScript modules that the dashboard imports as `synthesis-scenario-{a,b,c}.ts`. The same notebook also simulates the edge tier on the Kaggle T4 with Gemma 4 E2B against four real disaster images, proving the JSON contract holds end-to-end in a single run.
